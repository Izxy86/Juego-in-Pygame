...
def escribirEnPantalla(screen, palabra, posicion, tamano, color):
    defaultFont= pygame.font.Font( pygame.font.get_default_font(), 35)
...
def musicaAmbiente():
    pygame.mixer.music.load("efectoAmbiente.ogg")
...
def dibujar(screen, candidata, listaIzq, listaMedio, listaDer, posicionesIzq ,
    ...
    #Linea del piso
    pygame.draw.line(screen, (COLOR_PISO), (0, ALTO-70) , (ANCHO, ALTO-70), 5)

    #linea vertical
    pygame.draw.line(screen, (COLOR_COLUMNA1), (ANCHO//3, ALTO-70) , (ANCHO//3, 0), 5)

    #linea vertical
    pygame.draw.line(screen, (COLOR_COLUMNA2), (2*ANCHO//3, ALTO-70) , (2*ANCHO//3, 0), 5)
    ...
    for i in range(len(listaMedio)):
        screen.blit(defaultFont.render(listaMedio[i], 1, COLOR_LETRAS), posicionesMedio[i])
    for i in range(len(listaDer)):
        screen.blit(defaultFont.render(listaDer[i], 1, COLOR_LETRAS), posicionesDer[i])

    screen.blit(ren1, (320, 550))
